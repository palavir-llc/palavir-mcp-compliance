...
if (!API_KEY) {
  console.error(
    "[palavir-mcp-compliance] Missing PALAVIR_COMPLIANCE_API_KEY. " +
...
const server = new Server(
  { name: "palavir-compliance", version: "0.1.1" },
  { capabilities: { tools: {} } }
);
  ...
  tools: [
    {
      name: "screen_entity",
      description:
        "Screen one individual or entity against LEIE (OIG exclusion list), OFAC SDN (Treasury sanctions), " +
    ...
    },
    {
      name: "get_api_info",
      description:
        "Return API metadata: database record counts (LEIE, OFAC, SAM, state Medicaid), version, " +
    ...
    },
    {
      name: "get_api_stats",
      description:
        "Return current API usage statistics (records loaded per database, last refresh timestamps).",
      inputSchema: { type: "object", properties: {} },
    },
    {
      name: "health_check",
